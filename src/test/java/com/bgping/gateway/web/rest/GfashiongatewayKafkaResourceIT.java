package com.bgping.gateway.web.rest;
...
import com.bgping.gateway.config.KafkaProperties;
...
class GfashiongatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        GfashiongatewayKafkaResource kafkaResource = new GfashiongatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/gfashiongateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/gfashiongateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
